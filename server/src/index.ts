...
app.post('/api/interests/:id/report', async (req, res) => {
    try {
        const { id } = req.params;
        const { quantity, unitPrice, transferMethod, reporter } = req.body;
        
        if (!id || quantity === undefined || unitPrice === undefined || !transferMethod || !reporter) {
            return res.status(400).json({ error: 'Missing required fields for reporting sale' });
        }
        
        const result = await reportInterestSale(id, quantity, unitPrice, transferMethod, reporter);
        res.json(result);
    } catch (error: any) {
        console.error('Report interest sale error:', error);
        res.status(500).json({ error: error.message || 'Internal server error reporting sale' });
    }
});
